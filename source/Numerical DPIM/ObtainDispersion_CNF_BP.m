function [omega, nc, umax, Backbone, manifold] = ObtainDispersion_CNF_BP(parameters, mode, rho, kn, order, varargin)
n_kn = length(kn);
omega = cell(n_kn,1);
nc = omega;
umax = omega;
...
tic
Compact_ReducedModel = InvariantManifold_CNF_BP(model_DPIM);
toc
