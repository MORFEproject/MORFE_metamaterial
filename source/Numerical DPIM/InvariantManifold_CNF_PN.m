function ReducedModel = InvariantManifold_CNF_PN(Model, mode)
...
f{1} = Model.spectrum.L(Master,Master);                                            % First-order CNF
Psi{1} = Model.spectrum.Y(1:N,Master);                                             % First-order invariant manifold of displacement variables
Upsilon{1} = Model.spectrum.Y(N+1:end,Master);                              % First-order invariant manifold of velocity variables

%% Build the p-order CNF and invariant manifold using DPIM framework
for p_i = 2:n_p
I_pi = Indp{p_i};
n_Ipi = size(I_pi,2);
f{p_i} = sparse(n, n_Ipi);
Psi{p_i} = sparse(N, n_Ipi);
Upsilon{p_i} = sparse(N, n_Ipi);
...
for l_i = 1:n_Ipi
    ...
    Xi_I = - G_I-G_Im1-G_Ip1 - H_I- H_Im1- H_Ip1 - M*nu_I ...
    ...
    switch abs(na)
        case 0 & (mode == 1)
            ...
            Phf = A\R;
            Psi{p_i}(:,l_i) = Phf(1:N);
            f{p_i}(1:2,l_i) = Phf(N+1:end);
            Upsilon{p_i}(:,l_i) = theta_I*Phf(1:N) + Vr*Phf(N+1:end) + mu_I;
        ...
        otherwise
            ...
            Phf = A\Xi_I;
            Psi{p_i}(:,l_i) = Phf;
            Upsilon{p_i}(:,l_i) = theta_I*Phf + mu_I;
    end
end
end
ReducedModel.f = f;
